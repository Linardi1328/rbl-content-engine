...
10. for direct scheduled social publication, `docs/production/PHASE_5_DIRECT_SOCIAL_PUBLISHING.md`
11. `docs/phase-0-spec.md`
12. `docs/platform-intelligence.md`
13. for Topview production tasks, `docs/topview/WORKFLOW.md`
14. for Topview capability/tool routing, `docs/topview/TOOL_MAP.md`
15. for live Topview capability discovery, `docs/topview/DISCOVERY.md`
16. for the controlled preflight/reference pilot, `docs/topview/PHASE_1D_PILOT.md`
17. the active task/prompt
18. existing implementation conventions
...
- Human approval/decision remains required before publication or external action in Phase 0–4. Phase 5 is the explicitly authorized exception: a valid Phase 5 post manifest may publish automatically at its scheduled time after local validation and all platform-specific consent/compliance gates pass.
...
```text
approved/final RBL export
-> platform-specific export + metadata
-> local Phase 5 manifest validation
-> scheduled datetime
-> direct official platform API
-> publication receipt / provider state
```

Phase 5 may automatically publish a due manifest **without another owner prompt** when the manifest itself is valid and the platform allows that behavior. This authorization does not retroactively remove the human-review gates from Phase 0–4.

Hard requirements:

- no platform token, refresh token, client secret, or credential may enter tracked files;
- runtime queue, auth state, and publication receipts stay under ignored `.production/`;
- each platform mutation is isolated so a failure cannot cause already-successful platforms to post twice;
- mark a platform `IN_PROGRESS` before its first write and never blindly replay an uncertain write after restart;
- use timezone-aware ISO-8601 scheduled datetimes;
...
- automatic retries are prohibited after uncertain provider writes unless the provider exposes a safe status/idempotency path and the implementation uses it;
- TikTok submitted posts are reconciled by `publish_id` instead of re-submitted;
- captions, titles, privacy, comments/duet/stitch settings, and related post configuration come from the Phase 5 manifest and are not guessed at publish time.

Phase 5 publication is a new explicit owner authorization. Earlier text in Phase 0, Stage 3D, Phase 4A, and Phase 4C that prohibits automatic publishing remains authoritative **inside those historical phases only**.
