...
class Phase2BExperimentBaselineTests(unittest.TestCase):
    ...
    def test_fixture_produces_matched_baseline_comparisons(self):
        ...
        by_id = {item["experiment"]["id"]: item for item in payload["experiments"]}
        ...
        self.assertEqual(long["metric_comparisons"]["views"]["comparison"], "ABOVE_BASELINE")
        self.assertEqual(long["metric_comparisons"]["leads"]["median"], 4.0)
        self.assertEqual(long["metric_comparisons"]["leads"]["comparison"], "ABOVE_BASELINE")
        ...
        self.assertEqual(short["metric_comparisons"]["views"]["comparison"], "ABOVE_BASELINE")
    ...
    def test_duplicate_experiment_content_id_fails_closed(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ManifestError, "Duplicate experiment content_id"):
    ...
    def test_unknown_metric_and_route_fail_closed(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ManifestError, "unsupported metric"):
                load_experiments(path)
        data = json.loads(EXPERIMENTS.read_text(encoding="utf-8"))
        data["experiments"][0]["treatment"]["monetization_routes"] = ["MAGIC_MONEY"]
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "experiments.json"
            path.write_text(json.dumps(data), encoding="utf-8")
    ...
    def test_missing_current_data_is_explicit(self):
        ...
        item = next(x for x in payload["experiments"] if x["experiment"]["id"] == "exp-event-ops-short")
    ...
    def test_insufficient_baseline_is_not_overinterpreted(self):
        ...
        long = next(x for x in payload["experiments"] if x["experiment"]["id"] == "exp-event-ops-long")
        self.assertEqual(long["metric_comparisons"]["views"]["comparison"], "INSUFFICIENT_BASELINE")
    ...
    def test_analysis_is_deterministic(self):
        ...
        self.assertEqual(analyze(manifest, analytics, history), analyze(manifest, analytics, history))
